...
parser.add_argument("--model_path", default="./00_sota/coco/checkpoints/model_iter_90000.pth", type=str, help="model_path")
...
def build_validation(model=None, par=None, val_loader=None, device='cuda', args=None):
    ...
    cat_list=voc.class_list if args.num_classes <= 21 else coco.class_list
    model.eval()
                ...
                if args.save_cam:
                    ...
                    if not args.save_cls_specific_cam:
                        ...
                        for cam,idx in zip(resized_attr_maps, cls_lst):
                            ...
                            alpha = 0.6
                            cam_rgb = alpha*cam_rgb + (1-alpha)*img
                            imageio.imsave(os.path.join(args.cs_cam_dir, name[0] + f"_{cat_list[idx+1]}.jpg"), cam_rgb.astype(np.uint8))
...
def crf_proc():
    ...
    metrics_tab_crf = format_tabs_multi_metircs([crf_score], ["confusion","precision","recall",'iou'], cat_list=cat_list)
    logging.info("\n"+ metrics_tab_crf)
