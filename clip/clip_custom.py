...
class Attention(nn.Module):
    ...
    def cure_attn_with_diff(self,k,ex_feats,B,C,N,scale):

        k_resize = k.permute(0,1,3,2).reshape(B,C,N)[:,:,1:].reshape(B,C,20,20)
        k_resize_in = F.interpolate(k_resize, size=(64,64), mode="bilinear", align_corners=False).reshape(B,C,-1)
        k_resize_in = k_resize_in.reshape(B,self.num_heads, C // self.num_heads,4096)
        k_refine = torch.matmul(ex_feats.unsqueeze(0).unsqueeze(0),k_resize_in.permute(0,1,3,2))
        k_refine = k_refine.transpose(-2,-1).reshape(B,C,4096)
        k_refine = F.interpolate(k_refine.reshape(B,C,64,64), size=(20,20), mode="bilinear", align_corners=False).reshape(B,C,-1)
        k_refine = k_refine.reshape(B,self.num_heads, C // self.num_heads,400).transpose(-2,-1)
        k_refine = torch.cat([k[:,:,:1],k_refine],dim=2)
        ex_attn = (k_refine @ k_refine.transpose(-2, -1)) * scale

        # ex_attn = (ex_attn - torch.mean(ex_attn, dim=-1, keepdim=True) * 1) * 3
        # ex_attn[ex_attn < 0.0] = float('-inf')

        return ex_attn.softmax(dim=-1)
    

    def cure_attn_with_diff2(self,qkv,ex_feats):

        qkv_refined = torch.matmul(ex_feats.unsqueeze(0).unsqueeze(0).unsqueeze(0),qkv[:,:,:,1:,:]) # 3 B H N D
        norm_factor = ex_feats.unsqueeze(0).unsqueeze(0).unsqueeze(0).sum(dim=-1).unsqueeze(-1)  # B 1 HW
        qkv_refined = qkv_refined / (norm_factor + 1e-6)

        q, k, v = qkv_refined[0], qkv_refined[1], qkv_refined[2] 

        attn1 = (k @ k.transpose(-2, -1)) * self.scale
        attn1 = attn1.softmax(dim=-1)
        attn1 = self.attn_drop(attn1)

        attn2 = (q @ q.transpose(-2, -1)) * self.scale
        attn2 = attn2.softmax(dim=-1)
        attn2 = self.attn_drop(attn2)

        return 2*attn1+attn2
    ...
    def forward(self, x, return_weights, ex_feats):
        ...
        ##TODO
        if ex_feats is None:
            attn = (attn1).softmax(dim=-1) +  2 * (attn2).softmax(dim=-1)
            # attn = ((attn1).softmax(dim=-1) + (attn2).softmax(dim=-1) + (attn3).softmax(dim=-1)) / 3
        else:
            attn = ((attn1).softmax(dim=-1) + 2 * (attn2).softmax(dim=-1)) #+ ex_attn1
            attn_feats = attn[:,:,1:,1:] + ex_feats
            attn[:,:,1:,1:] = attn_feats
        ##TODO##
