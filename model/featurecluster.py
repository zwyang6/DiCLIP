...
class DFC_KL(nn.Module):
    ...
    def forward(self, x):
        ...
        ##cluster around the prototype iteratively
        ...
        # # plt.imshow(cluster.reshape(20, 20).cpu().numpy(),
        #            cmap='jet', alpha=0.5, vmin=-1, vmax=cluster.unique().shape[0])
        # plt.show()
        # plt.pause(0.1)
        # cluster = F.interpolate(cluster.float().reshape(56,56).unsqueeze(0).unsqueeze(0), size=[20, 20], mode='nearest')[0,0,...].reshape(-1)
        return cluster.to(torch.int64), len(idx)
...
def attn_kl_cluster(attn, num_cluster=9, iter=5, size=64, visual=False):
    grid = torch.arange(0, size, size // num_cluster ** 0.5)[None].long()
    grid = grid.T * size + grid

    attn = attn.T    # hw c
    attn = torch.where(attn == 0, 1e-6, attn)
    anchor, cluster = attn[grid.flatten()], None

    ##cluster around the anchor iteratively
    for _ in range(iter):
        dis = kl_dis(anchor, attn)
        values, cluster = torch.min(dis, dim=0)
        idx = cluster.unique()
        anchor = torch.stack([attn[cluster == dat].mean(0) for dat in idx], dim=0)
    idx = cluster.unique()
    for idx_ in idx:
        cluster[cluster == idx_] = torch.nonzero(idx == idx_).squeeze()
    ...
    if visual:
        ...
        plt.ion()
        plt.clf()
        plt.imshow(cluster.reshape(size, size).cpu().numpy(),
                   cmap='jet', alpha=0.5, vmin=-1, vmax=cluster.unique().shape[0])
        plt.show()
        plt.pause(0.1)
    # cluster = F.interpolate(cluster.float().reshape(56,56).unsqueeze(0).unsqueeze(0), size=[20, 20], mode='nearest')[0,0,...].reshape(-1)

    clusters = cluster.to(torch.int64)
    one_hot = F.one_hot(clusters, len(idx))
    cluster_att = one_hot[:, clusters].float() ##聚类完之后的mask的attn_map, 所以是bianry
    return cluster_att

def attn_kl_cluster2(attn, num_cluster=9, iter=5, size=64, visual=False):
    grid = torch.arange(0, size, size // num_cluster ** 0.5)[None].long()
    grid = grid.T * size + grid
