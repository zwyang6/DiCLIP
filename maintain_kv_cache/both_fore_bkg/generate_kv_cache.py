...
def build_validation(model=None, num_cluster=None, val_loader=None, device='cuda', args=None):
    ...
    model.eval()
    cache_key={f'{cls}': [] for cls in voc.class_list}
    cache_value={f'{cls}': [] for cls in voc.class_list}
    ...
    with torch.no_grad():
        ...
        if os.path.exists(raw_pth_file):
            ...
            for i, data in tqdm(enumerate(val_loader), total=len(val_loader), ncols=100, ascii=" >="):
                ...
                if masks_binary.sum() > 0:
                    ...
                    cls_labels_wbkg = torch.zeros((1,args.num_classes)).to(cls_labels.device)
                    cls_labels_wbkg[:,1:] = cls_labels
                    ...
                    cache_value[cls_name].append(cls_labels_wbkg)
        ...
        cluster_keys = []
        cluster_values = []
        for i, cls_name in enumerate(voc.class_list):
