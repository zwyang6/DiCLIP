...
from utils.affutils import refine_cams_with_aff, refine_cams_with_bkg_weclip, refine_cams_with_aff2
...
from model.ciaa import cia_aggregare, cia_aggregare_from_cam
...
parser.add_argument("--stage", default="generate_mask", type=str, help="infer_set")
...
def build_validation(model=None, par=None, val_loader=None, device='cuda', args=None):
    ...
    with torch.no_grad():
        for _, data in tqdm(enumerate(val_loader), total=len(val_loader), ncols=100, ascii=" >="):
            ...
            attr_maps_raw, attn_weights, diff_attn, diff_attn_down = model(inputs, label_aff)
            ...
            if args.training_free:
                ...
                attr_maps_raw, attn_weights = cure_attr_map_flip(model, inputs)
                # attr_maps_raw = attr_maps_raw
            ...
            for i, attr_map in enumerate(attr_maps_raw):
                ...
                refined_attr_maps_raw, cls_lst = refine_cams_with_aff(attr_map, attn_weight, cls_label, size=inputs.shape[2:], seg_attn=None, caa_thre=0.79)
                ...
                valid_lam, pred_mask = recursive_refinement_with_diff_attn(cls_labels, refined_attr_maps_raw, diff_attn, mask_size=labels.shape[-2:],iter=10)
                ...
                if args.save_cam:
                    ...
                    convert_test_seg2RGB(np.squeeze(pred_mask.cpu().numpy()).astype(np.uint8),args.segs_rgb_dir + "/" + name[0] + ".png")
...
def crf_proc():
    ...
    images_path = os.path.join(args.data_folder, 'images',)
    ...
    def _job(i):
        ...
        if "generate_mask" in args.stage:
            label = image[:,:,0]
        else:
        ...
        imageio.imsave(args.segs_crf_dir + "/" + name + ".png", np.squeeze(pred).astype(np.uint8))
...
if __name__ == "__main__":
    ...
    if args.crf_post:
        ...
        args.segs_crf_dir = os.path.join(base_dir, f"{args.infer_set}_{cpt_name}_{tag}_segs/seg_preds")
        args.segs_crf_rgb_dir = os.path.join(base_dir, f"{args.infer_set}_{cpt_name}_{tag}_segs/segcrf_preds_rgb")
        os.makedirs(args.logits_dir, exist_ok=True)
        os.makedirs(args.segs_crf_dir, exist_ok=True)
