...
import os
import sys
sys.path.append("./maintain_kv_cache/both_fore_bkg")
import numpy as np
import torch
...
parser.add_argument("--num_cluster", default=10, type=int, help="resize the long side")
parser.add_argument("--with_bkg", default=True, type=lambda x: x.lower() in ["true", "1", "yes"], help="take cam as seg")
#! TO DO
...
def validate(args=None):
    ...
    model.eval()
    text_embeddings = model.integral_text_features.cpu()
    ...
    build_validation(model=model, num_cluster=args.num_cluster, val_loader=val_loader, device='cuda', args=args, text_embeddings=text_embeddings)
    torch.cuda.empty_cache()

    return True

def build_validation(model=None, num_cluster=None, val_loader=None, device='cuda', args=None, text_embeddings=None):

    model.eval()
    start_idx = 0 if args.with_bkg else 1 
    cache_key={f'{cls}': [] for cls in voc.class_list[start_idx:]}
    cache_value={f'{cls}': [] for cls in voc.class_list[start_idx:]}

    with torch.no_grad():
        raw_pth_file = f'datasets/dif/diffusion_v2_1_kv_cache_raw_with_bkgxx.pth'
        if os.path.exists(raw_pth_file):
            cache_key, cache_value = torch.load(raw_pth_file)
            print(f'{raw_pth_file} loaded')
        else:
            bkg_tag = 'with_bkg' if args.with_bkg else 'no_bkg'
            tag = f'with_clustering_{num_cluster}_centroids_per_cls' if num_cluster is not None else 'no_clustering'
            print(f'generating kv_cache file with_cluseter_{args.num_cluster}_{bkg_tag}')
            for i, data in tqdm(enumerate(val_loader), total=len(val_loader), ncols=100, ascii=" >="):
                ...
                if masks_binary.sum() > 0:
                    ...
                    if args.with_bkg:
                        cls_labels = torch.zeros((1,args.num_classes)).to(cls_labels.device)
                        cls_labels[:,cls_idx+1] = 1
                    cache_value[cls_name].append(cls_labels)

                    ## add bkg key
                    if args.with_bkg:
                        cls_labels_bkg = torch.zeros((1,args.num_classes)).to(cls_labels.device)
                        cls_labels_bkg[:,0] = 1
                        key = feats[:,1:,:]*(~masks_binary)
                        key_norm = key.sum(1)/ (~masks_binary).sum(1)
                        cache_key['_background_'].append(key_norm)
                        cache_value['_background_'].append(cls_labels_bkg)

                else:
                    continue
            
            raw_kv_cache=[cache_key, cache_value]
            print(f'diffusion_v2_1_kv_cache_raw_{bkg_tag} saved.')
            torch.save(raw_kv_cache, f'/data/PROJECTS/Diffusion_CLIP/DiT_base_diffusion/datasets/dif/diffusion_v2_1_kv_cache_raw_{bkg_tag}.pth')

        cluster_keys = []
        cluster_values = []
        text_keys = text_embeddings[:(args.num_classes-1),...]
        for i, cls_name in enumerate(voc.class_list[start_idx:]):
            ...
            if num_cluster is not None:
                ...
                cluster_embedding = torch.tensor(cls_key)
                cluster_value = cls_value
            
            # add text as key
            # cluster_embedding = torch.cat([text_keys[i].unsqueeze(0), cluster_embedding])
            # cluster_value = torch.cat([cluster_value[0].unsqueeze(0), cluster_value])
        ...
        print(f'diffusion_v2_1_kv_cache_{tag}_{bkg_tag} saved.')
        torch.save(kv_cache, f'/data/PROJECTS/Diffusion_CLIP/DiT_base_diffusion/datasets/dif/diffusion_v2_1_kv_cache_{tag}_{bkg_tag}.pth')
