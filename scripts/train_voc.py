...
def train(args=None):
    ...
    for n_iter in range(args.max_iters):
        ...
        return_dynamic = True #if n_iter >= 4000 else False
        ...
        for i, attr_map in enumerate(attr_maps_raw):
            ...
            seg_attn = attn_pred[i,...].unsqueeze(0) if n_iter >= 4000 else None
            ...
            valid_lam, attr_aff_labels = recursive_refinement_with_diff_attn(cls_label, refined_attr_maps_raw, diff_attn[i], mask_size=labels.shape[-2:],iter=5)
        ...
        aff_mask = cams_to_affinity_label(seg_pred, mask=attn_mask) if n_iter >= 34000 else cams_to_affinity_label(aff_pseudos, mask=attn_mask)
        _, diver_pseudo_labels = lam_to_label(attr_maps_raw.permute(0,2,1).reshape(4,20,mask_size,mask_size), cls_labels.cuda(), img_box=None, ignore_mid=False, bkg_thre=args.bkg_thre, high_thre=args.high_thre, low_thre=args.low_thre, ignore_index=args.ignore_index)
