...
from utils.tbutils import make_grid_image, make_grid_label,  make_grid_image2
...
parser.add_argument("--cache_file", default="./datasets/dif_coco/diffusion_v2_1_kv_cache_with_clustering_5_centroids_per_cls_with_bkg_coco.pth", type=str, help="custom clip")
parser.add_argument("--adapter_size", default=512, type=int, help="adapter_size")
...
parser.add_argument("--eval_iters", default=10000, type=int, help="validation iters")
...
def train(args=None):
    ...
    for n_iter in range(args.max_iters):
        ...
        return_dynamic = True if n_iter >= 60000 else False
        attr_maps_raw, attn_weights, diff_attn, segs, attn_pred, dynamic_cam = model(inputs, return_dynamic)

        diff_cams = []
        for i, attr_map in enumerate(attr_maps_raw):
            cls_label = cls_labels[i]
            attn_weight = attn_weights[:,i,...]
            seg_attn = attn_pred[i,...].unsqueeze(0) if n_iter >= 40000 else None
            ...
            refined_attr_maps_raw = torch.stack(refined_attr_maps,dim=0).flatten(-2,-1).permute(1,0).unsqueeze(0)

            ###TODO recursive affinity refinment
            # diff_attn = extract_diff_attn(inputs[i][None], diff_attn_extractor)
            valid_lam, attr_aff_labels = recursive_refinement_with_diff_attn(cls_label, refined_attr_maps_raw, diff_attn[i], mask_size=labels.shape[-2:],iter=10)
            cams = valid_lam[0,1:][cls_lst,...]
            diff_cams.append(valid_lam[:,1:])

            aff_pseudos.append(attr_aff_labels)
        aff_pseudos = torch.cat(aff_pseudos,dim=0)
        diff_cams = torch.cat(diff_cams,dim=0)
        ...
        ### dynamic_loss
        dynamic_cam = F.interpolate(dynamic_cam, size=aff_pseudos.shape[1:], mode='bilinear', align_corners=False)
        dynamic_loss = get_seg_loss(dynamic_cam, aff_pseudos.type(torch.long), ignore_index=args.ignore_index)
        ...
        aff_mask = cams_to_affinity_label(seg_pred, mask=attn_mask) if n_iter >= 80000 else cams_to_affinity_label(aff_pseudos, mask=attn_mask)
        _, diver_pseudo_labels = lam_to_label(attr_maps_raw.permute(0,2,1).reshape(4,args.num_classes-1,mask_size,mask_size), cls_labels.cuda(), img_box=None, ignore_mid=False, bkg_thre=args.bkg_thre, high_thre=args.high_thre, low_thre=args.low_thre, ignore_index=args.ignore_index)
        diver_loss,_,_ = get_aff_loss(attn_pred, aff_mask)

        # warmup
        loss = 1.0 * seg_loss  + args.w_diver * diver_loss + 0.1 * dynamic_loss

        avg_meter.add({
            'seg_loss': seg_loss.item(),
            'diver_loss': diver_loss.item(),
            'dynamic_cam_loss': dynamic_loss.item(),
        })
        ...
        if (n_iter + 1) % args.log_iters == 0:
            ...
            if args.local_rank == 0:
                logging.info("Iter: %d; Elasped: %s; ETA: %s; LR: %.3e; seg_loss: %.4f, diver_loss: %.4f, dynamic_cam_loss: %.4f" % (n_iter + 1, delta, eta, cur_lr, avg_meter.pop('seg_loss'), avg_meter.pop('diver_loss'), avg_meter.pop('dynamic_cam_loss')))
                if tb_logger is not None:
                    grid_img1, grid_cam1 = make_grid_image(inputs.detach(), attr_maps_raw.detach(), cls_label=cls_labels.detach())
                    _, grid_cam2 = make_grid_image2(inputs.detach(), diff_cams.detach(), cls_label=cls_labels.detach())
                    ...
                    tb_logger.add_image("visual/diff_cam1", grid_cam2, global_step=global_step)
        ...
        if (n_iter + 1) % args.eval_iters == 0 and (n_iter + 1) >= 1:
            ...
            tab_results = build_validation(model=model, par=par, val_loader=val_loader, num_classes=args.num_classes, diff_attn_extractor=None)
            if args.local_rank == 0:
                logging.info("\n"+tab_results)
