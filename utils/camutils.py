...
def cam_to_label_iseg(cam, cls_label, bkg_thre=0.3, cls_thre=0.4):
    """
    Args:
    ...
    if bkg_thre > 0:
        ...
        reshape_cam = reshape_cam.reshape(b, c, h, w)
        valid_cam = cls_label_rep * reshape_cam
        valid_cam = torch.cat([torch.pow(1 - valid_cam.amax(dim=1, keepdim=True), 1), valid_cam], dim=1)
...
def cure_attr_map_flip(model, inputs, ex_fts=True, flip=True, raw_fts=None):
    ...
    with torch.no_grad():
        if flip:
            ...
            if ex_fts:
                ...
                attr_maps_raw, attn_weights = model(inputs_cat, ex_feats=ex_feats)
            else:
    ...
    return attr_maps_raw, attn_weights
...
def cure_attr_map(model, inputs, ex_feats):
    with torch.no_grad():
        attr_maps_raw, attn_weights = model(inputs, ex_feats=ex_feats)
