...
def _refine_cams(ref_mod, images, cams, valid_key,):
    ...
    # img_mst_tree = mst(images)
    # refined_cams = global_apro(refined_cams.float(), images.float(), img_mst_tree, sigma=0.01)
...
def refine_cams_with_aff(attr_map, attn_weights,cls_label, size,caa_thre =0.79, attn_layers=6, seg_attn=None):
    ...
    for i, cls in enumerate(cls_lst):
        ...
        # for _ in range(3):
        #     cross_attn = torch.matmul(trans_mat , cam_to_refine).reshape(h // 16, w // 16)
        #     # cross_attn = torch.where(cross_attn < cross_attn.amax(dim=-2, keepdim=True) * 0.1, 0, cross_attn)
        #     cam_refined = cross_attn
